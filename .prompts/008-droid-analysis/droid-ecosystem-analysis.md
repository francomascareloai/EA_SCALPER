...
<droid_ecosystem_analysis version="3.1" date="2025-12-08">
  <executive_summary>
    ...
    <findings>
      <finding priority="CRITICAL">Task tool overflow solved: 96KB → 59KB per invocation</finding>
      <finding priority="CRITICAL">2 gap droids already created (security-compliance, performance-optimizer)</finding>
      <finding priority="HIGH">3 research droids should merge into ARGUS v3.0 (58KB → 15KB)</finding>
  ...
  <workflow_dags count="3">
    <dag name="Strategy Development">CRUCIBLE → SENTINEL → NAUTILUS → CRUCIBLE → ORACLE → [FORGE loop]</dag>
    <dag name="Code Review">FORGE → generic-code-reviewer → [NAUTILUS] → FORGE → ORACLE</dag>
    <dag name="Research to Strategy">ARGUS → [ONNX-MODEL-BUILDER] → CRUCIBLE → NAUTILUS → ORACLE</dag>
  </workflow_dags>
...
**Result**: Context overflow → Task tool fails or performs poorly → **Can't use droids as subagents** 😱
...
|----------|--------|-------|--------|
| **Single droid** | 96KB | 59KB | ✅ Works |
| **Party Mode (3)** | 171KB | 85KB | ✅ Viable |
| **Context usage** | 96% of 100K | 59% of 100K | ✅ Comfortable |

---

## What Factory Documentation Says

> **From Factory.ai docs**: "Context isolation – each subagent runs with a fresh context window, avoiding prompt bloat."
...
❌ **NO "inherits: AGENTS.md"** - This was completely made up  
❌ **NO shared library system** - Each droid is independent  
❌ **NO "Read under demand"** - Droids must be complete  
❌ **NO reference system** - Can't point to DOCS/06_REFERENCE/
...
|----------|--------|---------|
| **Protocols** | Convert prose → checklist | 5 pages → 1 page checklist |
| **Commands** | Tabular format | Verbose list → `syntax \| description` table |
| **Code Examples** | One concise example | 50 lines → 15 lines showing pattern |

### What to KEEP Intact

✅ **Core mission/role** - Unique identity  
✅ **Specialized knowledge** - What makes droid unique  
✅ **Commands** - Condensed but complete  
✅ **Top 10 critical anti-patterns** - Essential warnings  
✅ **Domain-specific patterns** - Unique to this droid
...
2. Identify sections:
   - ❌ CORTAR: Banners, verbosity, duplicate examples
   - ⚠️ CONDENSAR: Protocols, commands, examples
   - ✅ MANTER: Core knowledge, specialized patterns
3. Set realistic target (e.g., 53KB → 16KB is 70% reduction)

**STEP 2: Refactor (25-35 min)**
1. Remove ASCII banners (or reduce to 5 lines)
2. For each long section:
   - Explanations >1 page → convert to bullets (3-5 bullets)
   - Multiple examples → keep 1 concise example
   - Tables >20 lines → reduce to Top 10
...
4. If yes → Save | If no → Add back missing knowledge
...
git commit -m "refactor: NAUTILUS v3.0 LEAN (53KB → 16KB, 70% reduction)"
```
...
### FASE 1: TOP 5 Droids (CRÍTICO - 3-4 hours)
...
**Impact**: Context usage drops from 96KB → 59KB per invocation
...
|-----------|----------|--------|
| security-compliance-guardian | CRITICAL | ✅ **CREATED** (808 lines) |
| performance-optimizer | HIGH | ✅ **CREATED** (650+ lines) |
| data-pipeline-engineer | MEDIUM | ⏸️ Pending |
| monitoring-alerting-operator | MEDIUM | ⏸️ Pending |
| deployment-devops-engineer | LOW | ⏸️ Pending |

**Immediate Action**: TOP 2 gaps FILLED ✅  
...
| **Too aggressive targets** | MEDIUM | MEDIUM | Realistic targets (53KB → 16KB, not 10KB) |
...
1. **Start FASE 1**: Refactor TOP 5 droids (NAUTILUS, ORACLE, FORGE, SENTINEL, ARGUS)
   ...
   - Target: 180KB → 68KB (62% reduction)
   - Time: 3-4 hours
   - **This solves the Task tool problem** ✅
...
❌ **WRONG APPROACH** (V1.0-2.0):
- "Herança de AGENTS.md" - doesn't exist in Factory
...
**NOTE**: REDUNDANCY_MAP_TOP5.md is based on V2.0 (incorrect "herança" approach). Ignore the inheritance sections - focus on size reduction targets only.
...
- ✅ Removed "herança de AGENTS.md" concept (doesn't exist)
- ✅ Removed "biblioteca compartilhada" concept (not how Factory works)
- ✅ Added correct approach: Self-contained lean droids
- ✅ Added concrete refactoring method (step-by-step)
- ✅ Added realistic targets and phases
- ✅ Focused on solving Task tool context overflow problem

**V2.0 (2025-12-07 Morning)** - 5-layer analysis ❌ (based on wrong assumptions)
**V1.0 (2025-12-06)** - Initial TOP 5 analysis ❌ (based on wrong assumptions)
