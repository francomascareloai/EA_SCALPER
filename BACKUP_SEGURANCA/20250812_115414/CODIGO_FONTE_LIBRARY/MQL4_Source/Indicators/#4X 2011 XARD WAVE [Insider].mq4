...
#property indicator_color5  DodgerBlue  //45,140,90
#property indicator_width5 1
#property indicator_color6  C'255,100,255'  //140,45,90
#property indicator_width6 1
#property indicator_color7  DodgerBlue  //45,140,90
#property indicator_width7 1
#property indicator_color8  C'255,100,255'  //140,45,90
#property indicator_width8 1
//--------------------------------------------------------------------
extern int    SR              = 3;  // =3..4
extern int    SRZZ            = 36; // =4..12..20..12
extern int    MainRZZ         = 36;//12;//20; // =12..20..54..20
extern int    FP              = 36;
...
SetIndexBuffer(4,Up);
SetIndexStyle(4,DRAW_ARROW,EMPTY);// Green up arrow - Зеленая метка вверх
SetIndexArrow(4,110);//159);
SetIndexEmptyValue(4,0.0);
SetIndexLabel(4,"trend UP");
SetIndexBuffer(5,Dn);
SetIndexStyle(5,DRAW_ARROW,EMPTY);//Red down arrow  - Красная метка вниз
SetIndexArrow(5,110);//159);
SetIndexEmptyValue(5,0.0);
SetIndexLabel(5,"trend DOWN");
SetIndexBuffer(6,pUp);
SetIndexStyle(6,DRAW_ARROW,EMPTY);//Green up markers - Зеленый маркер вверх
SetIndexArrow(6,110);//159);
SetIndexEmptyValue(6,0.0);
SetIndexBuffer(7,pDn);
SetIndexStyle(7,DRAW_ARROW,EMPTY);//Red down markers- Красный вниз маркер
SetIndexArrow(7,110);//159);
...
if(prevBars!=Bars) { SBZZ=Bars-nSBZZ; 
...
//---------------------------------------------------------------------
   ...
   if (alertsOn)
   {
      ...
      if (Up[whichBar] > 0) doAlert(" Тренд изменился на верх");
      if (Dn[whichBar] > 0) doAlert(" Тренд изменился на вниз");
   }   
   return(0);
}
...
void doAlert(string doWhat)
{
   ...
   string message;
      ...
      if (previousAlert != doWhat || previousTime != Time[0]) {
          ...
          message =  StringConcatenate(Symbol()," at ",TimeToStr(TimeLocal(),TIME_SECONDS)," WAVE ",doWhat);
             if (alertsMessage) Alert(message);
             if (alertsEmail)   SendMail(StringConcatenate(Symbol()," WAVE"),message);
